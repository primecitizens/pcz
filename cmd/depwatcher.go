...
func NewDepWatch(tc *buildcfg.Toolchain, initPkgs []buildcfg.Package, entryPkg string) *DepWatcher {
	dw := &DepWatcher{
		tc:       tc,
		entryPkg: entryPkg,
		dirs:     map[string]int{},
...
func (w *DepWatcher) CurrentDeps() []buildcfg.Package {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *DepWatcher) Contains(dir string) *buildcfg.Package {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *DepWatcher) Build(output string) (err error) {
	...
	return w.tc.Build(w.entryPkg, output)
}
...
func (w *DepWatcher) updateLocked(pkgs []buildcfg.Package) []error {
	var errs []error
