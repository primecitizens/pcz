...
func TestLess(t *testing.T) {
	for _, test := range tests {
		...
		case float64:
			b = cmp.FLess(test.x.(float64), test.y.(float64))
		}
		if b != (test.compare < 0) {
...
func TestCompare(t *testing.T) {
	for _, test := range tests {
		...
		case float64:
			c = cmp.FCompare(test.x.(float64), test.y.(float64))
		}
		if c != test.compare {
...
func TestSort(t *testing.T) {
	...
	for i := 0; i < len(input)-1; i++ {
		if cmp.FLess(input[i+1], input[i]) {
			t.Errorf("Less sort mismatch at %d in %v", i, input)
		}
		if cmp.FCompare(input[i], input[i+1]) > 0 {
