...
// indeSlice returns the index of the first instance of b in a, or -1 if b is not present in a.
// Requires 2 <= len(b) <= indexArgBMaxLen.
func indexSlice(a, b []byte) int
...
// IndexSlice returns the index of the first instance of sep in s,
...
func IndexSlice(s, sep []byte) int {
	...
	case n <= indexArgBMaxLen:
		...
		for i < t {
			if s[i] != c0 {
				// IndexSliceByte is faster than indexSlice, so use it as long as
			...
			fails++
			i++
			// Switch to indexSlice when IndexByte produces too many false positives.
			if fails > cutover(i) {
				r := indexSlice(s[i:], sep)
				if r >= 0 {
					return r + i
				}
				return -1
