...
func TestAddUintptr(t *testing.T) {
	...
	runParallel(N, iter, func() {
		atomic.AddUintptr(&total, inc)
		atomic.AddUintptr(&total, uintptr(-int64(inc)))
	})
	if total != 0 {
...
func TestAddUintptrOnUint64(t *testing.T) {
	if arch.BigEndian {
	...
	const inc = 100
	val := uint64(0)
	atomic.AddUintptr((*uintptr)(unsafe.Pointer(&val)), inc)
	if inc != val {
...
func TestUnaligned64(t *testing.T) {
	...
	shouldPanic(t, "Add64", func() { atomic.Add64(up64, 1) })
	shouldPanic(t, "Swap64", func() { atomic.Swap64(up64, 1) })
...
func TestBitwiseContended32(t *testing.T) {
	// Start with every bit in array set to 0.
	a := make([]uint32, 16)
	...
	for i := 0; i < 32; i++ {
		...
		go func() {
			for n := 0; n < N; n++ {
				for i := range a {
					atomic.Or32(&a[i], m)
					if atomic.Load32(&a[i])&m != m {
						t.Errorf("a[%v] bit %#x not set", i, m)
					}
					atomic.And32(&a[i], ^m)
					if atomic.Load32(&a[i])&m != 0 {
						t.Errorf("a[%v] bit %#x not clear", i, m)
					}
				}
			}
...
func TestCasRel32(t *testing.T) {
	...
	for j := 0; j < 32; j += 1 {
		...
		x.o = (1 << j) + 0
		x.n = (1 << j) + 1
		if !atomic.CasRel32(&x.i, x.o, x.n) {
...
func TestStorePointer(t *testing.T) {
	var p [2]*int
	for i := range p {
		atomic.StorePointer(unsafe.Pointer(&p[i]), unsafe.Pointer(new(int)))
	}
	if p[0] == p[1] {
		t.Error("Bad escape analysis of StorePointer")
	}
}
